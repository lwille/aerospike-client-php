...
################################################################################
#
# This script is used to detect if libaerospike.a is installed and to export
...
detect_linux()
{
  ...
  if [ ! -z "$(which lsb_release 2> /dev/null)" ]; then
    ...
    case ${DIST_NAME} in
      ...
      "centos6" | "centos7" | "redhatenterpriseserver6" | "fedora20" | "fedora21" | "oracleserver6" | "scientific6" )
        echo "el6" "rpm"
        return 0
        ;;
